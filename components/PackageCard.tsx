...
export default function PackageCard({ packageInfo, onFavoriteChange }: PackageCardProps) {
	...
	const formatNumber = (num: number) => {
		return new Intl.NumberFormat(i18n.language === 'tr' ? 'tr-TR' : 'en-US').format(num)
	}
				...
				<div className="space-y-6">
					...
					<div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
						<div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
							<div className="text-sm text-gray-500 dark:text-gray-400">{t('packageCard.releaseDate')}</div>
							<div className="font-medium dark:text-gray-200">{formatDate(packageInfo.publishedAt)}</div>
						</div>
						{packageInfo.downloads && (
							<div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
								<div className="text-sm text-gray-500 dark:text-gray-400">{t('packageCard.weeklyDownloads')}</div>
						...
						{packageInfo.score && (
							<>
								<div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
									<div className="text-sm text-gray-500 dark:text-gray-400">{t('packageCard.qualityScore')}</div>
									<div className="font-medium dark:text-gray-200">{Math.round(packageInfo.score.final * 100)}%</div>
								</div>
								<div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
									<div className="text-sm text-gray-500 dark:text-gray-400">{t('packageCard.popularity')}</div>
